...
using BlazorSozluk.Common.Exceptions;
...
namespace BlazorSozluk.Api.Application.Features.Commands.User
{
    public class LoginUserCommandHandler : IRequestHandler<LoginUserCommand, LoginUserViewModel>
    {
        ...
        public async Task<LoginUserViewModel> Handle(LoginUserCommand request, CancellationToken cancellationToken)
        {
            ...
            var paa = PasswordEncryptor.Encryp(request.Password);
            if(dbUser.Password != paa)
                throw new DatabaseValidationException("password is wrong");
            if(!dbUser.EmailConfirmed)
                throw new DatabaseValidationException("emailadress not confirmed yet");
